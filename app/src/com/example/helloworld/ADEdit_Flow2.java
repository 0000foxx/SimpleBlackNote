	...
	void initetMoney(){
		...
		etMoney.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable s) {
				// TODO Auto-generated method stub
				if(s.length()>7){
					Toast.makeText(ADEdit_Flow2.this, "最大輸入7位數字,請重新輸入",Toast.LENGTH_SHORT).show();
					s.clear();
				}
				else if(s.length()<=0){
					Toast.makeText(ADEdit_Flow2.this, "未填入數字,請重新輸入",Toast.LENGTH_SHORT).show();
				}
			}
		});
	}
	...
	void initetDetail() {
		...
		etDetail.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable s) {
				// TODO Auto-generated method stub
				if (s.length() <= 0) {
//					Toast.makeText(ADEdit_Flow.this, "未填入備註,請重新輸入",
